...
summary(penguins)
penguins$species
colnames(penguins) #gives names of just the columns or #names(penguins) 
colnames(penguins)[2]
colnames(penguins)[3]<- "BillLengthMm"
colnames(penguins)[4:6]<- c("BillDepthMm", "FlipperLengthMm", "BodyMassG")
colnames(penguins)
DataFunctionDivide(Score)toupper("hee hee)
tolower("WHY ARE YOU YELLING")
```
...
view(subset(penguins, penguins$species==("Chinstrap")))
```
can do math to fill in the columns
```{r}
penguins$Billinfo<-penguins$BillDepthMm + penguins$BillLengthMm
...
view(mtcars)
colnames(mtcars)
colnames(mtcars)[1:11]<- toupper(colnames(mtcars))
view(mtcars)
```
...
mtcars$tranmission3<- as.numeric(levels(mtcars$Transmission)[mtcars$Transmission])
```
...
multipleBy<- function(number1, number2){
  number1*number2
}
multipleBy(OneToTen, tenToOne)
numberDat<- cbind.data.frame(OneToTen, tenToOne)
numberDat$multiply<- multipleBy(OneToTen, tenToOne)
```
...
```{r}
mtcars$mpgDividehp<- MeanDivideSd(mtcars$mpg, mtcars$hp)
```
...
```{r}
#nrow is for a dataset, length is for lists, column, or vector
...
for (i in 1:nrow(ChickWeight)){
 if(ChickWeight$weight[i]>100){
    ChickWeight$ChickWeightMass[i]<-"yes"
  } else{ChickWeight$ChickWeightMass[i]<-"no"}
}
#this is a correct for loop
