...
```{r}
Animals <- c("Horse", "Cow", "Pig", "Chicken", "Dog", "Cat")
```
2)Make a vector of the number of cookies you think each animal could eat
```{r}
AnimalCookies <- c(101, 120, 304, 6, 10, 0)
```
...
```{r}
Animal_Data <- cbind.data.frame(Animals, AnimalCookies)
```
...
```{r}
FavoriteTv <- c("Supernatrual", "Andor", "Severance", "Daredevil")
FavTvRatings<- c(9, 10, 10, 7)
```

3.  Make these vectors into a data frame.

```{r}
Tv_Dataset <- cbind.data.frame(FavoriteTv, FavTvRatings)
```
